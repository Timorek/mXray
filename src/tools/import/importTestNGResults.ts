...
export const importTestNGResultsSchema = {
  xml_content: z.string().describe('TestNG XML results string'),
...
export async function importTestNGResults(
  ...
  try {
    ...
    const result = await xrayService.importTestNGResults(args.xml_content, queryParams);
    return {
    ...
    return {
      content: [{ type: 'text', text: `Error importing TestNG results: ${message}` }],
    };
  }
}
