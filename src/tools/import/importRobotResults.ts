...
export const importRobotResultsSchema = {
  xml_content: z.string().describe('Robot Framework XML results string'),
...
export async function importRobotResults(
  ...
  try {
    ...
    const result = await xrayService.importRobotResults(args.xml_content, queryParams);
    return {
    ...
    return {
      content: [{ type: 'text', text: `Error importing Robot Framework results: ${message}` }],
    };
  }
}
